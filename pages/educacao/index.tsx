import { FaBook, FaBullseye, FaCalculator, FaSignOutAlt, FaPiggyBank, FaMoon,FaSun } from "react-icons/fa";
...
export default function Educacao() {
  ...
  return (
    <div className="flex h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-colors duration-300">
      {/* Sidebar */}
      <aside className="w-64 bg-gray-100 dark:bg-gray-800 p-7 flex flex-col justify-between">
        <div>
          <h2 className="text-6xl font-bold mb-6 font-comfortaa">Orbs</h2>
          <nav className="flex flex-col gap-5">
            <Link href="/dashboard" className="flex items-center gap-2 text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 font-comfortaa text-xl">
              <FaBullseye /> Dashboard
            </Link>
            <Link href="/educacao" className="flex items-center gap-2 text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 font-comfortaa text-xl">
              <FaBook /> Educação
            </Link>
            <Link href="/metas" className="flex items-center gap-2 text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 font-comfortaa text-xl">
              <FaPiggyBank /> Metas
            </Link>
            <Link href="/calculadora" className="flex items-center gap-2 text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 font-comfortaa text-xl">
              <FaCalculator /> Calculadora
            </Link>
          </nav>
        </div>
        <Link href="/" className="flex items-center gap-4 text-2xl text-gray-700 dark:text-gray-300 hover:text-red-600">
          <FaSignOutAlt /> Sair
        </Link>
      </aside>
...
"
        <div className="flex justify-between items-center mb-20 mt-7 text-right">
          ...
          <h1 className="text-3xl md:text-5xl font-comfortaa leading-snug text-black dark:text-white">
            <span className="block text-indigo-500 ml-8">Aprenda mais <span className="text-black dark:text-white font-normal">sobre sua receita e</span>
            </span>
          ...
          <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-700 text-black-600 dark:text-gray-300 px-5 py-2 rounded-full shadow-sm font-comfortaa">
            <span className="text-sm truncate max-w-[150px] ">O que a IA tem a dizer...</span>
            <button
        ...
        <div className="w-full flex justify-center">
          <section className="flex flex-wrap gap-6 justify-center">
            {cards.map(({ imagem, largura, alturaImagem, alturaContainer }, i) => (
              <div
                key={i}
              ...
              >
                <div className={`bg-gradient-to-br from-blue-500 to-purple-700 rounded-xl w-full ${alturaContainer} flex items-center justify-center mb-4`}>
                  <img
                    src={imagem}
