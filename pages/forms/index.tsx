...
export default function Forms() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload = {
        userId: user.id,
        media_salarial: parseFloat(
          (answers.media_salarial || '0').replace(',', '.'),
        ).toFixed(2),
        idade: parseInt(answers.idade || '0', 10),
        quantidade_filhos:
          answers.filhos === '1' ? answers.quantidade_filhos || '0' : '0',
        dinheiro:
          answers.dinheiro === '7'
            ? parseFloat(
                (answers.dinheiro_outro || '0').replace(',', '.'),
              ).toFixed(2)
            : (() => {
      ...
      <div className="flex flex-col gap-4 py-6 md:py-8 ">
        ...
        <div className="flex flex-row mr-4 align-items-center">
          {currentIndex > 0 && (
            ...
            >
              <img
                src="/vector/back_arrow_forms.svg"
                alt="Voltar"
                className="w-8 h-8 xl:ml-4 xl:mt-4"
              />
            </button>
          )}
          ...
          {currentQuestion.tipo === 'input' ? (
            <>
              <InputText
                ...
                placeholder={
                  currentQuestion.id === 'media_salarial'
                    ? 'Ex: 4500.00'
                    : 'Ex: 30'
                }
              />
            ...
            <>
              ...
              {currentQuestion.id === 'dinheiro' &&
                answers['dinheiro'] === '7' && (
                  <div className="mt-2">
                    <label className="block mb-1 font-comfortaa">
                      Quantia*
                    </label>
                    <InputText
                      type="text"
