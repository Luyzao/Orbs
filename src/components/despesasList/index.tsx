...
export default function DespesasList() {
  return (
    <div className="bg-gray-50 px-6 pt-5 text-black rounded-xl shadow-xl ">
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          className="bg-gray-200 border-0 px-3 py-1  rounded-lg border border-gray-300 text-sm"
        />
      ...
      <div className="h-[370px] overflow-y-auto">
        {despesas.map((grupo, i) => (
          <div key={i} className="mb-4 ">
            <p className="text-sm text-gray-500 mb-2">{grupo.data}</p>
            {grupo.itens.map((item, j) => (
              <div
                key={j}
                className="flex justify-between items-center px-4 bg-white py-1 rounded-lg mb-2 shadow-sm"
              >
                <div className="flex items-center gap-2 w-auto">
                  <span
                    className={`w-2rem h-3 rounded-full ${item.cor}`}
                  ></span>
                  <span className="text-gray-700 w-full text-sm">
                    {item.nome}
                  </span>
                </div>
                <span className="text-gray-800 w-auto font-medium">
